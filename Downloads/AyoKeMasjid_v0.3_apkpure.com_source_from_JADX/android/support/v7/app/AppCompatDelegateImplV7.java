...
class AppCompatDelegateImplV7 extends AppCompatDelegateImplBase implements Callback, LayoutInflaterFactory {
    ...
    class C00991 implements Runnable {
        ...
        public void run() {
            ...
            if ((AppCompatDelegateImplV7.this.mInvalidatePanelMenuFeatures & Barcode.QR_CODE) != 0) {
    ...
    private static final class PanelFeatureState {
        ...
        void setStyle(Context context) {
            ...
            widgetTheme.resolveAttribute(C0107R.attr.actionBarPopupTheme, outValue, true);
            if (outValue.resourceId != 0) {
                widgetTheme.applyStyle(outValue.resourceId, true);
            }
            widgetTheme.resolveAttribute(C0107R.attr.panelMenuListTheme, outValue, true);
            if (outValue.resourceId != 0) {
                widgetTheme.applyStyle(outValue.resourceId, true);
            } else {
                widgetTheme.applyStyle(C0107R.style.Theme_AppCompat_CompactMenu, true);
            }
            Context context2 = new ContextThemeWrapper(context, 0);
            context2.getTheme().setTo(widgetTheme);
            this.listPresenterContext = context2;
            TypedArray a = context2.obtainStyledAttributes(C0107R.styleable.Theme);
            this.background = a.getResourceId(C0107R.styleable.Theme_panelBackground, 0);
            this.windowAnimations = a.getResourceId(C0107R.styleable.Theme_android_windowAnimationStyle, 0);
            a.recycle();
        }
        ...
        MenuView getListMenuView(MenuPresenter.Callback cb) {
            ...
            if (this.listMenuPresenter == null) {
                this.listMenuPresenter = new ListMenuPresenter(this.listPresenterContext, C0107R.layout.abc_list_menu_item_layout);
    ...
    class C06422 implements OnApplyWindowInsetsListener {
        C06422() {
        }
    ...
    class C06433 implements OnFitSystemWindowsListener {
        C06433() {
        }
    ...
    AppCompatDelegateImplV7(Context context, Window window, AppCompatCallback callback) {
        ...
        this.mInvalidatePanelMenuRunnable = new C00991();
    }
        ...
        if (!this.mSubDecorInstalled) {
            ...
            if (this.mWindowNoTitle) {
                if (this.mOverlayActionMode) {
                    this.mSubDecor = (ViewGroup) inflater.inflate(C0107R.layout.abc_screen_simple_overlay_action_mode, null);
                } else {
                    this.mSubDecor = (ViewGroup) inflater.inflate(C0107R.layout.abc_screen_simple, null);
                }
                if (VERSION.SDK_INT >= 21) {
                    ViewCompat.setOnApplyWindowInsetsListener(this.mSubDecor, new C06422());
                } else {
                    ((FitWindowsViewGroup) this.mSubDecor).setOnFitSystemWindowsListener(new C06433());
                }
            } else if (this.mIsFloating) {
                this.mSubDecor = (ViewGroup) inflater.inflate(C0107R.layout.abc_dialog_title_material, null);
                this.mOverlayActionBar = false;
                this.mHasActionBar = false;
            } else if (this.mHasActionBar) {
                Context themedContext;
                TypedValue outValue = new TypedValue();
                this.mContext.getTheme().resolveAttribute(C0107R.attr.actionBarTheme, outValue, true);
                if (outValue.resourceId != 0) {
                    themedContext = new ContextThemeWrapper(this.mContext, outValue.resourceId);
                } else {
                    themedContext = this.mContext;
                }
                this.mSubDecor = (ViewGroup) LayoutInflater.from(themedContext).inflate(C0107R.layout.abc_screen_toolbar, null);
                this.mDecorContentParent = (DecorContentParent) this.mSubDecor.findViewById(C0107R.id.decor_content_parent);
            ...
            if (this.mDecorContentParent == null) {
                this.mTitleView = (TextView) this.mSubDecor.findViewById(C0107R.id.title);
            }
            ...
            ContentFrameLayout abcContent = (ContentFrameLayout) this.mSubDecor.findViewById(C0107R.id.action_bar_activity_content);
    ...
    private void applyFixedSizeWindow(ContentFrameLayout contentFrameLayout) {
        ...
        TypedArray a = this.mContext.obtainStyledAttributes(C0107R.styleable.Theme);
        a.getValue(C0107R.styleable.Theme_windowMinWidthMajor, contentFrameLayout.getMinWidthMajor());
        a.getValue(C0107R.styleable.Theme_windowMinWidthMinor, contentFrameLayout.getMinWidthMinor());
        if (a.hasValue(C0107R.styleable.Theme_windowFixedWidthMajor)) {
            a.getValue(C0107R.styleable.Theme_windowFixedWidthMajor, contentFrameLayout.getFixedWidthMajor());
        }
        if (a.hasValue(C0107R.styleable.Theme_windowFixedWidthMinor)) {
            a.getValue(C0107R.styleable.Theme_windowFixedWidthMinor, contentFrameLayout.getFixedWidthMinor());
        }
        if (a.hasValue(C0107R.styleable.Theme_windowFixedHeightMajor)) {
            a.getValue(C0107R.styleable.Theme_windowFixedHeightMajor, contentFrameLayout.getFixedHeightMajor());
        }
        if (a.hasValue(C0107R.styleable.Theme_windowFixedHeightMinor)) {
            a.getValue(C0107R.styleable.Theme_windowFixedHeightMinor, contentFrameLayout.getFixedHeightMinor());
        }
        a.recycle();
        ...
        if (mode != null) {
            ...
            if (this.mActionModeView == null) {
                if (this.mIsFloating) {
                    ...
                    baseTheme.resolveAttribute(C0107R.attr.actionBarTheme, outValue, true);
                    if (outValue.resourceId != 0) {
                    ...
                    this.mActionModePopup = new PopupWindow(actionBarContext, null, C0107R.attr.actionModePopupWindowStyle);
                    this.mActionModePopup.setContentView(this.mActionModeView);
                    this.mActionModePopup.setWidth(-1);
                    actionBarContext.getTheme().resolveAttribute(C0107R.attr.actionBarSize, outValue, true);
                    ...
                    this.mShowActionModePopup = new C01004();
                } else {
                    ViewStubCompat stub = (ViewStubCompat) this.mSubDecor.findViewById(C0107R.id.action_mode_bar_stub);
                    if (stub != null) {
        ...
        if (!st.isOpen && !isDestroyed()) {
            ...
            if (cb == null || cb.onMenuOpened(st.featureId, st.menu)) {
                ...
                if (wm != null && preparePanel(st, event)) {
                    ...
                    st.isHandled = false;
                    WindowManager.LayoutParams lp2 = new WindowManager.LayoutParams(width, -2, st.f1x, st.f2y, Place.TYPE_ADMINISTRATIVE_AREA_LEVEL_2, 8519680, -3);
    ...
    private boolean initializePanelMenu(PanelFeatureState st) {
        ...
        if ((st.featureId == 0 || st.featureId == 8) && this.mDecorContentParent != null) {
            ...
            if (outValue.resourceId != 0) {
                ...
                widgetTheme.applyStyle(outValue.resourceId, true);
                widgetTheme.resolveAttribute(C0107R.attr.actionBarWidgetTheme, outValue, true);
            } else {
                baseTheme.resolveAttribute(C0107R.attr.actionBarWidgetTheme, outValue, true);
            }
            if (outValue.resourceId != 0) {
                if (widgetTheme == null) {
                    widgetTheme = context.getResources().newTheme();
            ...
            if (this.mActionModeView.isShown()) {
                ...
                if (mlp.topMargin != newMargin) {
                    ...
                    if (this.mStatusGuard == null) {
                        ...
                        this.mStatusGuard.setBackgroundColor(this.mContext.getResources().getColor(C0107R.color.abc_input_method_navigation_guard));
