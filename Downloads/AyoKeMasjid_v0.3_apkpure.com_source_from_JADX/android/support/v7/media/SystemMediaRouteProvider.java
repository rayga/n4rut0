...
abstract class SystemMediaRouteProvider extends MediaRouteProvider {
    ...
    static class JellybeanImpl extends SystemMediaRouteProvider implements Callback, VolumeCallback {
        ...
        public JellybeanImpl(Context context, SyncCallback syncCallback) {
            ...
            this.mUserRouteCategoryObj = MediaRouterJellybean.createRouteCategory(this.mRouterObj, context.getResources().getString(C0142R.string.mr_user_route_category_name), false);
            updateSystemRoutes();
        }
        ...
        public void onRouteSelected(int type, Object routeObj) {
            if (routeObj == MediaRouterJellybean.getSelectedRoute(this.mRouterObj, MediaRouterJellybean.ALL_ROUTE_TYPES)) {
        ...
        public void onSyncRouteAdded(RouteInfo route) {
            ...
            int index = findSystemRouteRecord(MediaRouterJellybean.getSelectedRoute(this.mRouterObj, MediaRouterJellybean.ALL_ROUTE_TYPES));
        ...
        protected void selectRoute(Object routeObj) {
            ...
            this.mSelectRouteWorkaround.selectRoute(this.mRouterObj, MediaRouterJellybean.ALL_ROUTE_TYPES, routeObj);
        }
    ...
    static class LegacyImpl extends SystemMediaRouteProvider {
        ...
        final class VolumeChangeReceiver extends BroadcastReceiver {
            ...
            public void onReceive(Context context, Intent intent) {
                if (intent.getAction().equals(VOLUME_CHANGED_ACTION) && intent.getIntExtra(EXTRA_VOLUME_STREAM_TYPE, -1) == LegacyImpl.PLAYBACK_STREAM) {
        ...
        final class DefaultRouteController extends RouteController {
            ...
            public void onSetVolume(int volume) {
                LegacyImpl.this.mAudioManager.setStreamVolume(LegacyImpl.PLAYBACK_STREAM, volume, 0);
                LegacyImpl.this.publishRoutes();
            }

            public void onUpdateVolume(int delta) {
                int volume = LegacyImpl.this.mAudioManager.getStreamVolume(LegacyImpl.PLAYBACK_STREAM);
                if (Math.min(LegacyImpl.this.mAudioManager.getStreamMaxVolume(LegacyImpl.PLAYBACK_STREAM), Math.max(0, volume + delta)) != volume) {
                    LegacyImpl.this.mAudioManager.setStreamVolume(LegacyImpl.PLAYBACK_STREAM, volume, 0);
                }
                LegacyImpl.this.publishRoutes();
            }
        ...
        private void publishRoutes() {
            ...
            int maxVolume = this.mAudioManager.getStreamMaxVolume(PLAYBACK_STREAM);
            this.mLastReportedVolume = this.mAudioManager.getStreamVolume(PLAYBACK_STREAM);
            setDescriptor(new MediaRouteProviderDescriptor.Builder().addRoute(new Builder(SystemMediaRouteProvider.DEFAULT_ROUTE_ID, r.getString(C0142R.string.mr_system_route_name)).addControlFilters(CONTROL_FILTERS).setPlaybackStream(PLAYBACK_STREAM).setPlaybackType(0).setVolumeHandling(1).setVolumeMax(maxVolume).setVolume(this.mLastReportedVolume).build()).build());
        }
    ...
    private static class JellybeanMr2Impl extends JellybeanMr1Impl {
        ...
        protected void selectRoute(Object routeObj) {
            MediaRouterJellybean.selectRoute(this.mRouterObj, MediaRouterJellybean.ALL_ROUTE_TYPES, routeObj);
        }
