...
public class LinearLayoutManager extends LayoutManager {
    ...
    class AnchorInfo {
        ...
        public boolean assignFromViewIfValid(View child, State state) {
    ...
    static class LayoutState {
        ...
        private View nextFromLimitedList() {
            ...
            int closestDistance = Strategy.TTL_SECONDS_INFINITE;
            for (int i = LinearLayoutManager.HORIZONTAL; i < size; i += LAYOUT_END) {
                ViewHolder viewHolder = (ViewHolder) this.mScrapList.get(i);
    ...
    static class SavedState implements Parcelable {
        ...
        static class C01491 implements Creator<SavedState> {
            C01491() {
            }
    ...
    class C08991 extends LinearSmoothScroller {
        C08991(Context x0) {
            super(x0);
        }
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView, State state, int position) {
        LinearSmoothScroller linearSmoothScroller = new C08991(recyclerView.getContext());
    ...
    public void onLayoutChildren(Recycler recycler, State state) {
        ...
        if ((state.getTargetScrollPosition() < this.mAnchorInfo.mPosition ? true : DEBUG) == this.mShouldReverseLayout) {
            extraForEnd = extra;
    ...
    private void layoutForPredictiveAnimations(Recycler recycler, State state, int startOffset, int endOffset) {
        if (state.willRunPredictiveAnimations() && getChildCount() != 0 && !state.isPreLayout() && supportsPredictiveItemAnimations()) {
            ...
            for (int i = HORIZONTAL; i < scrapSize; i += VERTICAL) {
                ...
                if (((scrap.getPosition() < firstChildPos ? true : DEBUG) != this.mShouldReverseLayout ? -1 : VERTICAL) == -1) {
            ...
            if (scrapExtraStart > 0) {
                ...
                layoutState = this.mLayoutState;
                layoutState.mCurrentPosition = (this.mShouldReverseLayout ? VERTICAL : -1) + layoutState.mCurrentPosition;
                fill(recycler, this.mLayoutState, state, DEBUG);
            }
            if (scrapExtraEnd > 0) {
                ...
                this.mLayoutState.mAvailable = HORIZONTAL;
                layoutState = this.mLayoutState;
                layoutState.mCurrentPosition = (this.mShouldReverseLayout ? -1 : VERTICAL) + layoutState.mCurrentPosition;
                fill(recycler, this.mLayoutState, state, DEBUG);
            }
    ...
    private View findFirstReferenceChild(int itemCount) {
        return findReferenceChild(HORIZONTAL, getChildCount(), itemCount);
    }
    ...
    public int findFirstVisibleItemPosition() {
        View child = findOneVisibleChild(HORIZONTAL, getChildCount(), DEBUG);
        return child == null ? -1 : getPosition(child);
    }

    public int findFirstCompletelyVisibleItemPosition() {
        View child = findOneVisibleChild(HORIZONTAL, getChildCount(), true);
        return child == null ? -1 : getPosition(child);
    }

    public int findLastVisibleItemPosition() {
        View child = findOneVisibleChild(getChildCount() - 1, -1, DEBUG);
        if (child == null) {
            return -1;
        }
