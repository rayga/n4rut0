...
class RoundRectDrawableWithShadow extends Drawable {
    ...
    RoundRectDrawableWithShadow(Resources resources, int backgroundColor, float radius, float shadowSize, float maxShadowSize) {
        ...
        this.mShadowStartColor = resources.getColor(C0108R.color.cardview_shadow_start_color);
        this.mShadowEndColor = resources.getColor(C0108R.color.cardview_shadow_end_color);
        this.mInsetShadow = resources.getDimensionPixelSize(C0108R.dimen.cardview_compat_inset_shadow);
    ...
    static float calculateVerticalPadding(float maxShadowSize, float cornerRadius, boolean addPaddingForCorners) {
        if (addPaddingForCorners) {
            return (float) (((double) (SHADOW_MULTIPLIER * maxShadowSize)) + ((Utils.AGGRESSIVE_ROUND_SEC - COS_45) * ((double) cornerRadius)));
        }
    ...
    static float calculateHorizontalPadding(float maxShadowSize, float cornerRadius, boolean addPaddingForCorners) {
        if (addPaddingForCorners) {
            return (float) (((double) maxShadowSize) + ((Utils.AGGRESSIVE_ROUND_SEC - COS_45) * ((double) cornerRadius)));
        }
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.translate(0.0f, this.mRawShadowSize / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
        drawShadow(canvas);
        canvas.translate(0.0f, (-this.mRawShadowSize) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
    ...
    private void drawShadow(Canvas canvas) {
        ...
        float inset = (this.mCornerRadius + ((float) this.mInsetShadow)) + (this.mRawShadowSize / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
        if (this.mCardBounds.width() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * inset) > 0.0f) {
            drawHorizontalEdges = true;
        } else {
            drawHorizontalEdges = false;
        }
        if (this.mCardBounds.height() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * inset) > 0.0f) {
        ...
        if (drawHorizontalEdges) {
            canvas.drawRect(0.0f, edgeShadowTop, this.mCardBounds.width() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * inset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        if (drawHorizontalEdges) {
            ...
            canvas2.drawRect(0.0f, edgeShadowTop, this.mCardBounds.width() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * inset), this.mShadowSize + (-this.mCornerRadius), this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
        ...
        if (drawVerticalEdges) {
            canvas.drawRect(0.0f, edgeShadowTop, this.mCardBounds.height() - (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * inset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
    ...
    private void buildShadowCorners() {
        ...
        this.mCornerShadowPaint.setShader(new RadialGradient(0.0f, 0.0f, this.mCornerRadius + this.mShadowSize, new int[]{this.mShadowStartColor, this.mShadowStartColor, this.mShadowEndColor}, new float[]{0.0f, startRatio, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT}, TileMode.CLAMP));
        this.mEdgeShadowPaint.setShader(new LinearGradient(0.0f, (-this.mCornerRadius) + this.mShadowSize, 0.0f, (-this.mCornerRadius) - this.mShadowSize, new int[]{this.mShadowStartColor, this.mShadowStartColor, this.mShadowEndColor}, new float[]{0.0f, 0.5f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT}, TileMode.CLAMP));
        this.mEdgeShadowPaint.setAntiAlias(false);
    }
    ...
    float getMinWidth() {
        return ((this.mRawMaxShadowSize + ((float) this.mInsetShadow)) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * Math.max(this.mRawMaxShadowSize, (this.mCornerRadius + ((float) this.mInsetShadow)) + (this.mRawMaxShadowSize / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)));
    }

    float getMinHeight() {
        return (((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) + ((float) this.mInsetShadow)) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * Math.max(this.mRawMaxShadowSize, (this.mCornerRadius + ((float) this.mInsetShadow)) + ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)));
    }
