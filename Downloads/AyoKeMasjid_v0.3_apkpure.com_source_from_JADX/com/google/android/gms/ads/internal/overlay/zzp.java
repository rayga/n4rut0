...
public class zzp {
    ...
    public zzp(Context context, VersionInfoParcel versionInfoParcel, String str, zzcd com_google_android_gms_internal_zzcd, zzcc com_google_android_gms_internal_zzcc) {
        this.zzBq = new zzb().zza("min_1", Double.MIN_VALUE, Utils.AGGRESSIVE_ROUND_SEC).zza("1_5", Utils.AGGRESSIVE_ROUND_SEC, 5.0d).zza("5_10", 5.0d, Utils.DEF_IMSAAK_INTERVAL).zza("10_20", Utils.DEF_IMSAAK_INTERVAL, 20.0d).zza("20_30", 20.0d, Utils.DEFAULT_ROUND_SEC).zza("30_max", Utils.DEFAULT_ROUND_SEC, Double.MAX_VALUE).zzgz();
        this.zzBB = -1;
    ...
    String zza(TextureView textureView) {
        ...
        while (i < 8) {
            ...
            for (int i2 = 0; i2 < 8; i2++) {
                ...
                j3 |= (Color.green(pixel) + (Color.blue(pixel) + Color.red(pixel)) > Barcode.ITF ? 1 : 0) << ((int) j);
                j--;
            }
            i++;
            j2 = j;
